...
function App() {
  ...
  const openaiAudioSpeech = async (): Promise<
    ReadableStream<Uint8Array> | undefined
  > => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_OPEN_AI_URL}/audio/speech`,
        {
          method: "POST",
          headers: {
  ...
  const handleTextToSpeech = async () => {
    try {
      ...
      setIsLoading(true);
      const aiSpeech = await openaiAudioSpeech();
      if (aiSpeech) {
        const reader = aiSpeech.getReader();
